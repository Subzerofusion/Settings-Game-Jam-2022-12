...
public class KAction {
  ...
  public float AwfulHack { get; set; } = 1f;
  ...
  public KAction Clone() {
    var clone = new KAction() {
      Id = this.Id,
      Name = this.Name,
      GoodForSelf = this.GoodForSelf,
      AwfulHack = this.AwfulHack,
      Cost = this.Cost,
      Damage = this.Damage,
      Description = this.Description,
      Effect = this.Effect,
      PerTurn = this.PerTurn,
      UsePrint = this.UsePrint
    };
    return clone;
  }
  ...
  public static KAction[] Attacks = new KAction[] {
    ...
    new KAction(){ Id = "atk_stab", Name = "Stab them in the Throat",
      ...
      Effect = (root2d, action, user, target) => {
        var stabs = target.Effects.Where(x=>x.Item3.Id == "fx_stab");
        if(stabs.Count() > 0) {
          stabs.First().Item3.Damage++;
        } else {
          target.Effects.Add((user == root2d.Player ? Target.Player : Target.Enemy, target == root2d.Player ? Target.Player : Target.Enemy, Effects.First(x=>x.Id=="fx_stab").Clone()));
        }
    ...
    new KAction(){ Id = "atk_pos_charge", Name = "Positive Slash",
      ...
      Damage = 10, Cost = 0,
      Effect = (root2d, action, user, target) => {
        target.Effects.Add((user == root2d.Player ? Target.Player : Target.Enemy, target == root2d.Player ? Target.Player : Target.Enemy, Effects.First(x=>x.Id=="fx_pos").Clone()));
    ...
    new KAction(){ Id = "atk_neg_charge", Name = "Negative Stab",
      ...
      Damage = 400, Cost = 0,
      Effect = (root2d, action, user, target) => {
        target.Effects.Add((user == root2d.Player ? Target.Player : Target.Enemy, target == root2d.Player ? Target.Player : Target.Enemy, Effects.First(x=>x.Id=="fx_neg").Clone()));
    ...
    new KAction(){ Id = "atk_knife_game", Name = "Knife Game",
      ...
      Effect = (root2d, action, user, target) => {
        if(new Random().Next(0, 10) == 0) {
          ...
          user.Effects.Add((user == root2d.Player ? Target.Player : Target.Enemy, user == root2d.Player ? Target.Player : Target.Enemy, Effects.First(x=>x.Id=="fx_knife_miss").Clone()));
    ...
    new KAction(){ Id = "atk_deep", Name = "Tendrils of the Deep",
      ...
      Damage = 10, Cost = 30,
        Effect = (root2d, action, user, target) => {
          target.Effects.Add((user == root2d.Player ? Target.Player : Target.Enemy, target == root2d.Player ? Target.Player : Target.Enemy, Effects.First(x=>x.Id=="fx_tendrils").Clone()));
    ...
    new KAction(){ Id = "atk_gatekeep", Name = "GateKeep",
      ...
      Damage = 10, Cost = 0,
      Effect = (root2d, action, user, target) => {
        KAction gatekeep = Effects.First(x=>x.Id=="fx_gatekeep").Clone();
        gatekeep.AwfulHack = user.Resistance;
        user.Effects.Add((user == root2d.Player ? Target.Player : Target.Enemy, user == root2d.Player ? Target.Player : Target.Enemy, gatekeep));
        user.Resistance = 0f;
    ...
    new KAction(){ Id = "atk_girlboss", Name = "Girlboss",
      ...
      Effect = (root2d, action, user, target) => {
        var attack = target.Attacks[new Random().Next(0, target.Attacks.Count())];
    ...
    new KAction(){ Id = "atk_signal", Name = "Signal",
      ...
      Effect = (root2d, action, user, target) => {
        target.Effects.Add((user == root2d.Player ? Target.Player : Target.Enemy, target == root2d.Player ? Target.Player : Target.Enemy, Effects.First(x=>x.Id==new string[]{"fx_health","fx_mana","fx_res"}[new Random().Next(0, 3)]).Clone()));
        return new string[]{$"{target.Name}'s resistances fall"};
      }},
    ...
    new KAction(){ Id = "atk_facts", Name = "Facts",
      ...
      Effect = (root2d, action, user, target) => {
        var dmg = Math.Max(0, target.Health - target.Mana);
        target.TrueDmg(dmg);
    ...
    new KAction(){ Id = "atk_logic", Name = "Logic",
      ...
      Effect = (root2d, action, user, target) => {
        var dmg = Math.Max(0, target.Mana - target.Health);
        target.TrueDmg(dmg);
    ...
    new KAction(){ Id = "atk_7_female_wives", Name = "Seven Female Wives",
      ...
      Effect = (root2d, action, user, target) => {
        user.Effects.Add((user == root2d.Player ? Target.Player : Target.Enemy, user == root2d.Player ? Target.Player : Target.Enemy, Effects.First(x=>x.Id=="fx_female_wife").Clone()));
    ...
    new KAction(){ Id = "atk_bongcloud", Name = "Bong Cloud",
      ...
      Effect = (root2d, action, user, target) => {
        target.Effects.Add((user == root2d.Player ? Target.Player : Target.Enemy, target == root2d.Player ? Target.Player : Target.Enemy, Effects.First(x=>x.Id=="fx_bonged").Clone()));
  ...
  public static KAction[] ForbiddenItems = new KAction[] {
   new KAction(){ Id = "item_curse_amulet", Name = "Cursed Amulet of Decimation",
      ...
      Effect = (root2d, action, user, target) => {
        ...
        target.Effects.Add((user == root2d.Player ? Target.Player : Target.Enemy, target == root2d.Player ? Target.Player : Target.Enemy,  Effects.First(x=>x.Id=="fx_curse").Clone()));
  ...
  public static KAction[] Items = new KAction[] {
    ...
    new KAction(){ Id = "item_molotov", Name = "Molotov Cocktail",
      ...
      Effect = (root2d, action, user, target) => {
        user.Items.Remove(action);
        target.Effects.Add((user == root2d.Player ? Target.Player : Target.Enemy, target == root2d.Player ? Target.Player : Target.Enemy, Effects.First(x=>x.Id=="fx_molotovd").Clone()));
  ...
  public static KAction[] Effects = new KAction[] {
    ...
    new KAction() { Id = "fx_stab", Name = "Stab Wound",
      ...
      PerTurn = (root2d, action, opponent, holder) => {
        int dmg = 0;
        for(int i = -1; i < action.Damage; i++) {
          dmg += new Random().Next(200, 600);
        }
        dmg = holder.StdDmg(dmg);
        return new string[]{$"{ holder.Name } lost {dmg} health from bleeding" };
      }
    },
    new KAction() { Id = "fx_gatekeep", Name = "Gate What Which be Kept",
      PerTurn = (root2d, action, opponent, holder) => {

        var toBeRemoved = holder.Effects.First(x=>x.Item3 == action);
        if(toBeRemoved.Item3.AwfulHack == 0) {
          return new string[0];
        } else {
          holder.Resistance = toBeRemoved.Item3.AwfulHack;
          toBeRemoved.Item3.AwfulHack = 0;
          // i can't remove it :(
          return new string[]{$"{ holder.Name } is no longer gatekeeping" };
        }
      }
    },
    ...
    new KAction() { Id = "fx_bonged", Name = "Bonged" },
...
public class Killable {
  ...
  public void SyncActions() {
    ...
    var concat = new List<KAction>();
    concat.AddRange(KAction.Items);
    concat.AddRange(KAction.ForbiddenItems);
    Items = ItemIds.Select(x => concat.First(y => y.Id == x)).ToList();
