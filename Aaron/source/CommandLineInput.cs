...
public class CommandLineInput : LineEdit
{
	...
	public bool MailRemoved = false;
	public bool MailSafeRemoved = false;
	...
	public void HomeCommands(string input)
	{
		switch (input)
		{
			case string s when !PlayerDead && IsValidCommand(new string[]{"go", "travel", "head", "move"}, s, CommandTestOption.StartsWith)
							&& IsValidCommand(new string[]{"north", "south", "east", "west", "up", "down", "left", "right", "forwards", "backwards", "back", "front", "towards", "toward", "away"}, s, CommandTestOption.Contains):
				MovePlayer(s);
				break;
			case string s when !PlayerDead && IsValidCommand(new string[]{"open", "inspect", "interact", "check", "read"}, s, CommandTestOption.Contains) 
							&& IsValidCommand(new string[]{"mailbox", "mail", "box", "letter"}, s, CommandTestOption.Contains):
				OpenMailbox(input);
				break;
	...
	public void GrueLocationCommands(string input)
	{
		switch(input)
		{
			...
			case string s when !PlayerDead && IsValidCommand(new string[]{"kill", "defeat", "attack", "damage"}, s, CommandTestOption.Contains):
				...
				{
					AppendToCommandHistory("It dawns on you that nothing else remains in this world but you and this lifeless creature, trapped forever in an endless field of nothingness. You point the tip of your sword at your stomach and, with one final breath, plunge it deep into your abdomen. Darkness fills your vision, and your consciousness fades, leaving only an empty husk to forever stand in the field forever");
					AppendToCommandHistory("You win?");
					this.Editable = false;
				}
				else
	...
	public void GenericCommands(string input)
	{
		switch(input)
		{
			...
			case string s when !PlayerDead && IsValidCommand(new string[]{"kill", "defeat", "attack", "damage"}, s, CommandTestOption.Contains):
				...
				break;
			case string s when !PlayerDead && IsValidCommand(new string[]{"fuck", "shit", "penis", "pussy", "cunt", "asshole", "piss", "fucking", "dick", "prick", "shithead", "fuckwit", "dumbass", "whore", "bitch"}, s, CommandTestOption.Contains):
	...
	public void DeleteObject(string input)
	{
		...
		{
			if((new List<string>(){"grue", "enemy", "creature", "monster"}).Any(m => m == objectToDelete))
			{
				AppendToCommandHistory($"{objectToDelete} has been removed.");
				GrueRemoved = true;
			}
			else if((new List<string>(){"player", "me", "myself", "self"}).Any(m => m == objectToDelete))
			{
				FakeCrash();
			}
			else if((new List<string>(){"mail", "mailbox", "box", "letter"}).Any(m => m == objectToDelete))
			{
				AppendToCommandHistory($"{objectToDelete} has been removed.");
				MailRemoved = true;
			}
	...
	public void OpenMailbox(string input)
	{
		if(MailRemoved)
		{
			if(MailSafeRemoved)
			{
				AppendToCommandHistory("You open the mailbox. Nothing is inside");
			}
			else
			{
				GenericCommands(input);
